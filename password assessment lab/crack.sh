...
HC_PATH = "/mnt/d/downloads/2022-12/hashcat-6.2.6"
HC_CMD = ${HC_PATH}/hashcat.bin
HC_OPT = "--workload-profile 2 --optimized-kernel-enable --hash-type 1000 --username --outfile-format 1,2,3,4,5,6"
HASH_FILE = "test.ntlm"
DICT = "/mnt/d/downloads/words/collected.txt"
...
HC_SESS = "d14-mask"
${HC_CMD} ${HC_OPTIONS} --outfile "${HC_SESS}" --session "${HC_SESS}" --attack-mode 3 --increment --increment-min 8 --increment-max 14 ${HASH_FILE} "?d?d?d?d?d?d?d?d?d?d?d?d?d?d"
...
HC_SESS = "markov"
${HC_CMD} ${HC_OPTIONS} --outfile "${HC_SESS}" --session "${HC_SESS}" --attack-mode 0 ${HASH_FILE}
